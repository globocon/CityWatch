...
namespace CityWatch.Web.Pages.Admin
{
    public class GuardSettingsModel : PageModel
    {
        ...
        public async Task<JsonResult> OnGetHRDescriptionBanDetailsAsync(int DescriptionID)
        {
        ...
        public JsonResult OnPostSaveGuardComplianceandlicanseNew(GuardComplianceAndLicense guardComplianceandlicense)
        {
            ...
            return new JsonResult(new { status, dbxUploaded, msg });
        }
        ...
        private bool UploadGuardLicenseToDropbox(GuardLicense guardLicense)
        {
            ...
            return _viewDataService.UploadDocumentToDropbox(fileToUpload, dbxFilePath);
        }
        ...
        public JsonResult OnPostSaveHRSettings(int Id, int hrGroupId, int refNoNumberId, int refNoAlphabetId, string description, int[] Selectedsites, 
            string[] SelectedStates, HrDateType dateType,bool isAllClientTypeMandatory,bool isAllStateMandatory)
        {
            var status = true;
            ...
            try
            {
                HrSettings hrSettingsnew = new HrSettings()
                {
                    ...
                    Description = description,
                    DateType = dateType,
                    IsAllClientTypeEnabled = isAllClientTypeMandatory,
                    IsAllStateEnabled = isAllStateMandatory
                };

                if (isAllStateMandatory)
                {
                    SelectedStates = _guardLogDataProvider.GetStates().Select(x => x.Name).ToArray();
                }
        ...
        public JsonResult OnPostUploadGuardAttachmentForRPLCertificates()
        {
            var success = true;
            var files = Request.Form.Files;

            var guardId = Request.Form["guardId"];
            int certificateId = Convert.ToInt32(Request.Form["certificateId"]);

            var fileName = string.Empty;

            int hrIdInt = _configDataProvider.GetCourseCertificateDocuments().Where(x => x.Id == certificateId).FirstOrDefault().HRSettingsId;
