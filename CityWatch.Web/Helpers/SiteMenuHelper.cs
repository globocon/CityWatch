...
namespace CityWatch.Web.Helpers
{
    public static class SiteMenuHelper
    {
        ...
        public static string GetMenuHtml(string pageName)
        {
            ...
            if (AuthUserHelper.LoggedInUserId != 74)
            {
            ...
            else
            {
                ...
                {
                    menuHtml.AppendLine("<div>");
                    menuHtml.AppendLine(@"<a href=""/Guard/Login?t=gl"" class=""nav-link py-0""><i class=""fa fa-user mr-2""></i>Login</a>");
                    menuHtml.AppendLine("</div>");
                }
                if (pageName == PageNameHelper.Index || pageName == PageNameHelper.Notify || pageName == PageNameHelper.DailyGuardLog || pageName == PageNameHelper.VehicleAndKeyLog)
                {
