        ...
        columns: [
            ...
            { field: 'notes', title: 'Event / Notes', width: 450 },
            {
                field: 'guardInitials', title: 'Guard Initials', width: 80, renderer: function (value, record) {
                    // Start - Hide initials for Control Room Messages
                    if (record.rcPushMessageId) {
                        return '';
                    }
                    // End - Hide initials for Control Room Messages
    ...
    if (gridGuardLog) {
        ...
        gridGuardLog.on('dataBound', function (e, records, totalRecords) {
            if (isControlRoomLogBook) {
                ...
                // Check records for PlayNotificationSound
                var shouldPlay = false;
                records.forEach(function (record) {
                    if (record.playNotificationSound === true || record.PlayNotificationSound === true) {
                        if (!audioplayedlist.includes(record.id)) {
                            shouldPlay = true;
                            audioplayedlist.push(record.id);
                            showNotificationSlider(record);
                        }
                    }
                });
        ...
        function showNotificationSlider(log) {
            var sliderId = 'guardNotificationSlider_' + log.id;
            if ($('#' + sliderId).length === 0) {
                var sliderHtml = `
                    <div id="${sliderId}" class="notification-slider" style="position: fixed; bottom: 20px; right: 20px; background-color: #f8d7da; color: #721c24; padding: 15px; border: 1px solid #f5c6cb; border-radius: 5px; box-shadow: 0 0 10px rgba(0,0,0,0.1); z-index: 9999; width: 300px;">
                        <strong>Control Room Message:</strong><br/>
                        ${log.notes}
                        <button type="button" class="close" onclick="$('#${sliderId}').remove()" style="position: absolute; top: 5px; right: 10px;">&times;</button>
                    </div>
                `;
                $('body').append(sliderHtml);
                // Auto hide after 10 seconds? User didn't specify, but let's keep it persistent until acknowledged or closed.
            }
        }
        // Project 4 , Task 48, Audio notification, Added By Binoy -- End
    }
