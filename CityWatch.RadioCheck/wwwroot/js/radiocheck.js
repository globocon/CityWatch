...
function checkUnreadMessages() {
    var logBookId = $('#ClientSiteLogBookId').val(); // Try to find this ID
    if (logBookId && logBookId != 0) {
        $.ajax({
            type: "GET",
            url: "/RadioCheck/RadioCheckV2?handler=CheckUnreadMessages",
            data: { logBookId: logBookId },
            success: function (data) {
                if (data && data.length > 0) {
                    data.forEach(function (log) {
                        if (!audioplayedlist.includes(log.id)) {
                            playNotificationSound();
                            showNotificationSlider(log);
                            audioplayedlist.push(log.id);
                        }
                    });
                }
            }
...
function playNotificationSound() {
    audioNotification.play().catch(error => console.log("Audio play failed: ", error));
}

function showNotificationSlider(log) {
    // Create and append slider HTML if not exists
    var sliderId = 'notificationSlider_' + log.id;
    if ($('#' + sliderId).length === 0) {
        var sliderHtml = `
            <div id="${sliderId}" class="notification-slider" style="position: fixed; bottom: 20px; right: 20px; background-color: #f8d7da; color: #721c24; padding: 15px; border: 1px solid #f5c6cb; border-radius: 5px; box-shadow: 0 0 10px rgba(0,0,0,0.1); z-index: 9999; width: 300px;">
                <strong>Guard Reply:</strong><br/>
                ${log.notes}
                <button type="button" class="close" onclick="acknowledgeMessage(${log.id}, '${sliderId}')" style="position: absolute; top: 5px; right: 10px;">&times;</button>
            </div>
        `;
...
window.acknowledgeMessage = function (logId, sliderId) {
    $.ajax({
        type: "POST",
        url: "/RadioCheck/RadioCheckV2?handler=AckMessage",
        data: { guardLogId: logId },
