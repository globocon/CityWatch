...
{
    public class KpiCustomWandScheduleViewModel
    {
        ...
        [Required]
        public CustomWandReportType? CustomWandReportType { get; set; }
        ...
        {
            return new KpiSendCustomWandSchedules()
            {
                Id = viewModel.Id,
                KpiSendCustomWandClientSites = viewModel.ClientSiteIds.Select(z => new KpiSendCustomWandClientSites() { CustomWandScheduleId = viewModel.Id, ClientSiteId = z }).ToList(),
                StartDate = viewModel.StartDate.Value,
                EndDate = viewModel.EndDate,
                Frequency = viewModel.Frequency.Value,
                CustomWandReportType = viewModel.CustomWandReportType.Value,
                Time = viewModel.Time,
        ...
        public static KpiCustomWandScheduleViewModel FromDataModel(KpiSendCustomWandSchedules dataModel)
        {
            return new KpiCustomWandScheduleViewModel()
            {
                Id = dataModel.Id,
                ClientSiteIds = dataModel.KpiSendCustomWandClientSites.Select(z => z.ClientSiteId).ToArray(),
                ClientTypes = string.Join(", ", dataModel.KpiSendCustomWandClientSites.Select(z => z.ClientSite.ClientType.Name).Distinct()),
                ClientSites = string.Join(", ", dataModel.KpiSendCustomWandClientSites.Select(z => z.ClientSite.Name).Distinct()),
                StartDate = dataModel.StartDate,
                EndDate = dataModel.EndDate,
                Frequency = dataModel.Frequency,
                CustomWandReportType = dataModel.CustomWandReportType,
                Time = dataModel.Time,
